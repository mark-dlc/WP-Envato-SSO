...
if ( isset( $_GET['code_background'] ) && ! empty( $_GET['code_background'] ) ) {
	...
	if ( isset( $tokens->error ) ) {
		...
		$user_name = json_decode( $response['body'] );
        }
	...
	if ( ! $user_id && email_exists( $user_email->email ) == false ) {
		...
		$user            = get_user_by( 'login', $user_name->username );
        ...
        wp_die( 'Single Sign On with Envato Failed.' );
	exit;
}