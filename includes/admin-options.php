...
class WPOSSO_Admin {
	...
	public function options_do_page() {
		...
		?>
		<div class="wrap">
			<h2>Single Sign On (Envato) Configuration</h2>
			<p>This plugin is meant to be used with the <a href="https://build.envato.com">Envato API</a>.</p>
			<div>
				<strong>Creating an App</strong>
				<ol>
					<li>Visit the <a href="https://build.envato.com">Envato API</a></li>
				...
				<p>
					When activated, this plugin adds a 'Login with Envato' button to the login screen. If you want to add a
						...
						<td>
							<input type="checkbox" name="<?php echo $this->option_name ?>[redirect_to_dashboard]"
							       value="1" <?php echo $options['redirect_to_dashboard'] == 1 ? 'checked="checked"' : ''; ?> />
                                        ...
                                        <tr valign="top">
						<th scope="row">Only allow users that purchased one of your items to sign in</th>
						<td>
							<input type="checkbox" name="<?php echo $this->option_name ?>[buyers_only]"
							       value="1" <?php echo $options['buyers_only'] == 1 ? 'checked="checked"' : ''; ?> />
						</td>
					</tr>
				</table>
	...
	public function validate( $input ) {
		$input['redirect_to_dashboard'] = isset( $input['redirect_to_dashboard'] ) ? $input['redirect_to_dashboard'] : 0;
                ...
                $input['buyers_only'] = isset( $input['buyers_only'] ) ? $input['buyers_only'] : 0;
